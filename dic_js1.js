...
ex) 1 + '대하' // '1대하' 
ex) 'banana' + 2 // 'banana2' 브라우저가 숫자 + 문자열을 자동으로 문자열로 바꿔줌
ex) 4 * 'apple' // NaN  +연산자가 아닌 다른 연산자를 사용했을 때 NaN으로 형변환을 하는 것도 브라우저가 자동으로 한 일임;
...
Number('3월')
// NaN
parseInt는 argument를 2개 받을 수 있음 parseInt('문자열', 10); 여기서 10이 뭐냐 10진법을 말함 (몇 진법을 사용해서 숫자로 표현할 건지 상황에 맞춰 2진법 16, 8 등 다 가능); 최대 36진수
parseInt나 Number나 받는 인자에 숫자가 아예 없다? NaN을 반환한다. 
